...
def make_api_request(url, params, max_retries=3):
    ...
    for attempt in range(max_retries):
        try:
            ...
            # Return response even if error (handled by your logic)
            else:
                return resp
...
@st.cache_data(ttl=300) # Caching added to save API calls
def get_stock_data_polygon(sym, lookback_days):
    ...
    try:
        resp = make_api_request(url, params) # Use safe wrapper
...
def get_option_price_polygon(sym, expiry, strike, option_type):
    ...
    try:
        ...
        # Check for API errors or None response
        if not resp:
